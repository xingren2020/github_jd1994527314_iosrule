...
hd_nm=['NC@京东农场','NS@年兽','MC@萌宠','JC@惊喜工厂','DC@京东工厂','ZD@种豆','MH@盲盒','JN@京喜农场','GJ@京东国际']
hd_me=['ME@注册圈友人数']
...
def bot_admin(id,mlist,pop):
  try:
    ...
    if id!=tg_admin_id or id!=tg_second_id:
       return 
    if pop==2 or pop==1:
      if mlist[0]=='/admin_viewcode':
        ...
        if str(mlist[1])==system[0] and id==tg_admin_id:
             reboot=2
             postmsg='重启命令:正确'
        else:
              postmsg='重启命令:错误'
        ...
        if str(mlist[1])==system[1] and id==tg_admin_id:
...
def read_sec(secret):
   ...
   return globalid
def ac_load():
   global tg_bot_id,tg_member_id,tg_group_id,tg_bot_cmd,tg_new_id,tg_admin_id,tg_ac_id,tg_second_id
   ...
   tg_second_id=read_sec('tg_second_id')
